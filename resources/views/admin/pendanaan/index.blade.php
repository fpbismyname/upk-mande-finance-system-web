<x-layouts.admin-app title="Saldo Pendanaan">
                        ...
                        @if (count($datas_catatan_pendanaan) > 0)
                            @foreach ($datas_catatan_pendanaan as $item)
                                ...
                                <tr @if ($current_account) class="bg-primary/50" @endif>
                                    ...
                                    <td
                                        class="{{ $item->tipe_catatan === App\Enums\Admin\CatatanPendanaan\EnumCatatanPendanaan::INFLOW ? 'text-success' : 'text-error' }}">
    ...
    <dialog id="modal-topup-saldo" class="modal">
        <div class="modal-box">
            ...
            <form action="{{ route('admin.pendanaan.tambah-saldo') }}" method="POST" class="flex flex-col p-4 gap-4">
                @csrf
                @method('post')
                ...
                <fieldset class="fieldset">
                    ...
                    <textarea name="catatan" class="textarea w-full validator" required>Topup saldo pendanaan</textarea>
                    @error('catatan')
                        <small class="text-error">{{ $message }}</small>
                    @enderror
                </fieldset>
        ...
        <div class="modal-box">
            ...
            <form action="{{ route('admin.pendanaan.tarik-saldo') }}" method="POST" class="flex flex-col p-4 gap-4">
                ...
                <fieldset class="fieldset">
                    ...
                    <p class="validator-hint hidden">
                        Jumlah tarik saldo minimal Rp 100.000
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">Catatan</legend>
                    <textarea name="catatan" class="textarea w-full validator" required>Tarik saldo pendanaan</textarea>
                    @error('catatan')
                        <small class="text-error">{{ $message }}</small>
                    @enderror
                </fieldset>
