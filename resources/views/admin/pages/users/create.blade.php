<x-layouts.admin-app title="Tambah akun pengguna" :breadcrumbs="$breadcrumbs">
    <div class="flex flex-col gap-4">
        {{-- Add form --}}
        <form action="{{ route('admin.users.add') }}" class="grid grid-cols-1 gap-2 md:grid-cols-2" method="POST">
            @csrf
            @method('post')
            {{-- nik --}}
            ...
            <div class="grid md:col-span-2">
                <fieldset class="fieldset">
                    ...
                    <p class="validator-hint hidden">
                        {{ __('validation.required', ['attribute' => 'Role']) }}
                    </p>
                    @error('role_id')
                        <p class="fieldset-label">{{ $message }}</p>
                    @enderror
