<x-layouts.client-app title="Edit pengajuan pinjaman">
    ...
    <div class="flex flex-col">
        <form action="{{ route('client.pengajuan-pinjaman.update', [$pengajuan_pinjaman->id]) }}"
            ...
            <fieldset class="fieldset">
                <legend class="fieldset-legend">
                    Nominal pengajuan
                    <small id="preview-nominal">{{ $pengajuan_pinjaman->formatted_nominal_pinjaman }}</small>
                </legend>
                <input type="number" min="1000000" max="{{ $kelompok->limit_pinjaman }}" name="nominal_pinjaman"
                    ...
                    value="{{ $pengajuan_pinjaman->nominal_pinjaman ?? old('nominal_pinjaman') }}" required />
            ...
            <!-- File Proposal -->
            <fieldset class="fieldset">
                <legend class="fieldset-legend">File Proposal (Opsional)</legend>
            ...
            <fieldset class="fieldset">
                ...
                <select name="tenor" class="select w-full validator">
                    <option value="" selected disabled>Pilih tenor</option>
                    @foreach ($list_tenor as $value => $key)
                        <option value="{{ $value }}" @selected(($pengajuan_pinjaman->tenor->value ?? old('tenor')) == $value)>{{ $key }}</option>
                    @endforeach
                </select>
                <p class="validator-hint hidden">
            ...
            <div class="grid place-items-center md:col-span-2">
                <div class="flex flex-row gap-4">
                    ...
                    <a href="{{ route('client.pengajuan-pinjaman.show', [$pengajuan_pinjaman->id]) }}"
